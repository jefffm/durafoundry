...
export async function cleanupFixtureRepository(
  ...
  const marker = await readFixtureMarker(markerPath, fixtureRoot);
...
async function readFixtureMarker(
  markerPath: string,
  fixtureRoot: string,
): Promise<{ fixtureRepoId?: string }> {
  const markerText = await readFile(markerPath, 'utf8');
  try {
    const marker = JSON.parse(markerText) as unknown;
    if (!isFixtureMarker(marker)) {
      throw new Error('marker does not contain a fixtureRepoId');
    }
    return marker;
  } catch (cause) {
    throw new Error(`Refusing to clean fixture with invalid marker: ${fixtureRoot}`, { cause });
  }
}
...
function isFixtureMarker(value: unknown): value is { fixtureRepoId?: string } {
  return (
    typeof value === 'object' &&
    value !== null &&
    'fixtureRepoId' in value &&
    typeof (value as { fixtureRepoId: unknown }).fixtureRepoId === 'string'
  );
}
